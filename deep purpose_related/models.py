...
def repurpose(X_repurpose, target, model, drug_names = None, target_name = None, 
	...
	with open(fo, 'w') as fout:
		...
		#print('target$$$ ', target)
		#print('model.target_encoding $$$$ ', model.target_encoding)
		#print(df_data)       
		...
		if drug_names is not None:
			...
			for i in range(len(X_repurpose)):
				...
				if model.binary:
					cols = ['Name', 'Target','Probability','Binding Score']
				else:
					cols = ['Name', 'Target','Binding Score']                    
				df_results = pd.DataFrame(columns=cols)
				len1 = len(print_list)
				path1 = os.path.join(result_folder, 'repurposing')
				for i in range(0,len1):
					df_results.loc[i] = print_list[i][0]
				df_results.to_csv(path1+".csv")
		if convert_y:
	...
	with open(fo,'w') as fout:
		...
		if drug_names is not None and target_names is not None:
			...
			for i in range(len(target)):
				#print('model binary## ',model.binary);
				if model.binary:
					if y_pred[i] > 0.62:
				...
				df_results = pd.DataFrame(columns=cols)
				len1 = len(print_list)
				path1 = os.path.join(result_folder, 'virtual_screening')
				for i in range(0,len1):
					df_results.loc[i] = print_list[i][0]
				df_results.to_csv(path1+".csv")
