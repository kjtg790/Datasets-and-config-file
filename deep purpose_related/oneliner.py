...
def repurpose(target, target_name = None, 
	...
	with open(fo, 'w') as fout:		
		...
		if drug_names is not None:
			...
			#print("print_list#1 ",print_list)
		cols = ['Name', 'Target','Binding Score']
		df_results = pd.DataFrame(columns=cols)
		rand_num = randint(10000, 9999999)
			
		len1 = len(print_list)
		path1 = os.path.join(result_folder_path, 'repurposing_')
		for i in range(0,len1):
			df_results.loc[i] = print_list[i][0]
		df_results.to_csv(path1+str(rand_num)+".csv")
